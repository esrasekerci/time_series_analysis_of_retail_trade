...
```{r}
## Loading data set
df <- read_csv("C:/Users/ibrah/Desktop/SeriesReport-202301180847.csv")
```
...
gglagplot(df2) + theme(axis.text.x = element_text(angle=90))
```
...
We can Use either 0.5246817-th power of the series or do y^05 transformations since the λ is very close to 0.5. It was decided to use the form with lambda value (this step is noted to be asked later).
...
```{r}
autoplot(df.t, ylab="$ (Billions)", main="Time Series Plot of Monthly Retail Trade Sales in USA", col="red")+theme_minimal()
```
...
```
```{r}
p1<-ggAcf(dif,lag.max=100) #seasonal difference
...
```
```{r}
p1<-ggAcf(dif,lag.max=100) #both seasonal and regular difference
p2<-ggPacf(dif,lag.max=100)
grid.arrange(p1,p2,nrow=1)
```
...
In order to remove stochastic trend, we apply differencing method on seasonal lags. To take the difference of the series, we use diff() function.
...
```{r}
fit1<-Arima(df.t,order = c(2, 1, 3))
fit1
```
The coefficient estimates are all highly significant, and we proceed to check further on this model.

```{r}
fit2<-Arima(df.t,order = c(3, 1, 3))
fit2
```

```{r}
fit3<-Arima(df.t,order = c(0, 1, 1), seasonal = c(0, 1, 1))
fit3
```

```{r}
fit4<-Arima(df.t,order = c(0, 0, 1), seasonal = c(0, 1, 1))
fit4
```
...
```{r}
spec=ugarchspec(variance.model=list(model = "fiGARCH",submodel="GARCH",garchOrder=c(1,1)),
                mean.model=list(armaOrder=c(1,1)),distribution.model="norm") 
def.fit1 = ugarchfit(spec, data=df.t)
def.fit1
```
...
def.fit2= ugarchfit(spec, data = df.t, solver = 'hybrid')
def.fit2
```
...
bootp=ugarchboot(def.fit2,method=c("Partial")[1],n.ahead = 60,n.bootpred=1000,n.bootfit=1000)
plot(bootp,which=2)
```
```{r}
bootp
...
```{r}
sfinal <- spec
setfixed(sfinal) <- as.list(coef(def.fit2))
